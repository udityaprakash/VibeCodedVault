...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'appearance' | 'models' | 'ai' | 'updates' | 'about'>('appearance');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const fetchReadme = async () => {
        if (window.api && window.api.getReadmeContent) {
          try {
            const content = await window.api.getReadmeContent();
            setReadmeContent(content || '');
          } catch (err) {
            console.error('Failed to load readme:', err);
          }
        } else {
          setReadmeContent('PromptVault is a local-first Electron desktop app...');
        }
      };
      fetchReadme();
    }
      ...
      <div className="glass-panel w-full max-w-2xl rounded-2xl border border-obsidian-850 bg-obsidian-900 shadow-2xl overflow-hidden flex flex-col h-[520px]">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-[180px] border-r border-obsidian-850 bg-obsidian-950/30 p-3 flex flex-col gap-1 shrink-0">
            ...
            >
              <Info size={14} className={activeTab === 'about' ? 'text-cyber-cyan' : ''} />
              About
            </button>
          </div>
          ...
          <div className="flex-1 p-6 overflow-y-auto space-y-6">
            ...
            {activeTab === 'about' && (
              <div className="space-y-4 animate-in fade-in duration-100 select-text">
                <div>
                  <h4 className="text-xs font-bold text-cyber-violet uppercase tracking-wider mb-3">About PromptVault</h4>
                  <div className="bg-obsidian-950/40 border border-obsidian-850 rounded-xl p-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                    {readmeContent ? (
                      renderMarkdown(readmeContent)
                    ) : (
                      <p className="text-xs text-obsidian-400">Loading documentation...</p>
                    )}
                  </div>
                </div>
              </div>
